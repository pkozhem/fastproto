...
impl Descriptor {
    ...
    /// Let Python's cyclic GC see the class references we hold. Without this the
    /// cycle `class -> __fastproto__ (Descriptor) -> refs -> class` is invisible
    /// to the collector and every referenced message/enum class leaks.
    fn __traverse__(&self, visit: PyVisit<'_>) -> Result<(), PyTraverseError> {
        for obj in self.refs.values() {
            visit.call(obj)?;
        }
        Ok(())
    }

    /// Drop the class references when the collector breaks a cycle.
